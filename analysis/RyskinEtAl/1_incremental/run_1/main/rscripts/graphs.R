...
nrow(toplot)
names(toplot)
toplot2 = left_join(toplot_select, toplot_eye, by=c("cond","pragContext","window","Region", "timebin")) %>% 
  mutate(window = fct_relevel(window,"prior","adjective")) %>% 
  mutate(ymin_selections = as.numeric(ymin_selections),ymax_selections = as.numeric(ymax_selections),prop_selections = as.numeric(prop_selections))
nrow(toplot)
names(toplot)
...
ggplot(toplot, aes(x=prop_selections,y=prop_looks,color=Region,shape=pragContext,group=1)) +
  ...
  xlim(0,1) +
  ylim(0,1) 
ggsave("../graphs/corr_overall_faceted_by_window.pdf",width=6.5,height=3.5)
ggsave("../graphs/corr_overall.pdf",width=5.5,height=3.5)

#TODO report these correlations (w/ facets, report the correlations with it)
#facet by window
...
# correlations split up by window
# .97 for prior
# 0.40 for adjective
# 0.99 for noun
...
# <fct>     <fct>             <dbl>   <dbl>
#   1 prior     no_contrast        1    0      
# 2 prior     contrast           0.94 0.00476
# 3 adjective no_contrast       -0.94 0.00599
# 4 adjective contrast           0.78 0.0664 
# 5 noun      no_contrast        1    0      
# 6 noun      contrast           0.98 0.00042
...
# <fct>     <fct>             <dbl>   <dbl>
#   1 prior     reliable           0.96 0.00217
# 2 prior     unreliable         0.98 0.00061
# 3 adjective reliable           0.55 0.258  
# 4 adjective unreliable         0.07 0.889  
# 5 noun      reliable           0.98 0.00048
# 6 noun      unreliable         0.99 0.00006
...
ggsave("../graphs/ryskin_fig2c_recreated.png",width=5.5,height=3.5)
