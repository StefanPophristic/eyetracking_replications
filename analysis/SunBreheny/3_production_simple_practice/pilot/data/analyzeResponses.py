...
#
# The code accepts "rubber" as a valid alternative to "eraser" because participants
#   can be British
#
# This code assumes "click on the boy with an orange" is a false statement when
#   e.g. the boy has two oranges --> YOU SHOULD CHANGE THIS, BECUASE THIS IS A
#   TECHNICALLY TRUE STATEMENT
#
# #### HOW TO USE THE SCRIPT ####
#
# This script is ready to be used. The only things that should be changed are
#   line 70, specifying the name of the csv file that should be imported, and
#   line 287, specifying the name of the csv file that will be exported.
#
...
#   e.g. "spon" instead of spoons, or participants using unexpected words, e.g.
...
df = pd.read_csv(r'3_production_extra_practice_pilot-trials.csv')
...
for a in range(0, numRows):
    ...
    elif targetNoun[a].rstrip("s") == "eraser" and "rubber" in responses[a]:
        correctNoun.append("1")
    else:
        correctNoun.append("0")
...
# the following code checks that if there are two images of apples on the screen
#   the participant correctly used the determiners "some" and "all" by not ignoring
#   the total number of the target object by just focusing on the target image
#   and ignoring the other instances of the target object
#   For example, if the participant wants to communicate that the target image is
#   of a boy with two apples, but in the middle of the screen there is an additional
#   apple, the utterance "click on the boy with all the apples" is false.
...
for a in range(0, numRows):
    ...
    if (currentDet == "one" or currentDet == "a"):
...
df.to_csv('3_production_extra_practice_pilot-trials_preprocessed.csv')


# I copied and pasted the above code to rerun it with the other csv file


import pandas as pd

# read in csv file of responses
df = pd.read_csv(r'3_production_simple_practice_pilot-trials.csv')
...
# Export the data frame as a csv file
df.to_csv('3_production_simple_practice_pilot-trials_preprocessed.csv')
