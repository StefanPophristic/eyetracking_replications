...
prior_df = read_tsv(paste0(getwd(),"/../../../shared/Ryskin_original_eyetracking_data/PragTrain3_baseline_window_timesummLongForm.txt"),
                    col_names=c('target_dur','contrast_dur','compet_dur','other_dur','currsubj','subject','trialnum','order','timebin','trialID','condWith','condBet','targ_loc','contrast_loc','compet_loc','list','trialType','accuracy','target_fix','target_AR1','compet_fix','compet_AR1', 'list')) %>% 
...
toplot_eye = pragtrain3_crit %>% 
  ...
  select(contrast_cond,prag_context_cond,trial_type,quarter, noun, feature, list, timebin,target_fix,compet_fix) %>% #,
  ...
  group_by(contrast_cond,prag_context_cond,window,trial_type,quarter,feature, noun, list, Region) %>%
...
toplot_select =  d_test %>%
  select(workerid,pragContext,cond,click_prior,click_adj,click_noun,loc_target_pic,loc_competitor_pic,loc_contrast,loc_big_filler,loc_small_filler,instruction,trial_number,trial,trialType, quarter,noun, feature, list) %>%
  ...
  mutate(other = case_when(target + competitor == 0 ~ 1,
                                TRUE ~ 0)) %>% 
  group_by(cond,pragContext,window, trialType, quarter, feature, noun, list) %>% #added trialType
  ...
  select(cond,pragContext,window,trialType,quarter,feature, noun, list, info_target,info_competitor,info_other) %>% #added trialType
  ...
  select(cond, pragContext, window, Region, trialType, quarter, feature, noun, list, prop_selections, ymin_selections, ymax_selections)
...
df_merged = left_join(toplot_select, toplot_eye, by=c("cond","pragContext","trialType","quarter","noun","feature","window","Region", "list")) %>%
...
write.csv(df_merged,"../data/analysis_by_quarter_data.csv", row.names = FALSE)
