...
dfDet <- dfDet %>%
  group_by(experiment, size, figure, noun, detUsed) %>%
  count() %>%
  ungroup()

# Add in all the size x noun x determiner combinations which were not observed
allCombinations <- unique(expand.grid(c(1,2), dfDet$size, c("boy", "girl"), dfDet$noun, c("all", "some", "num", "noDet", "other")))
allCombinations <- allCombinations %>%
  rename(experiment = Var1, size = Var2, figure = Var3, noun = Var4, detUsed = Var5)

dfDet <- right_join(dfDet, allCombinations, by = c("experiment", "size", "figure", "noun", "detUsed"))
...
totalSums <- dfDet %>%
  group_by(experiment, size, figure, noun) %>%
  summarize(sum = sum(n))

# Merge the two together
dfDet <- merge(dfDet, totalSums, by = c("experiment", "size", "figure", "noun"))
...
surprisalMeanAndCI <- dfDet %>%
  ...
  group_by(experiment, size, figure, detUsed) %>%
  summarize(
...
graphSurprisalCorrExp1 <- dfCorr %>% 
  filter(experiment == 1) %>%
  ggplot(aes(x=surprisal, y=correlation, color=determiner, shape = size,group=1)) + 
  geom_point(size = 4) +
  geom_smooth(method="lm",se=F,color="gray80",alpha=.5) +
  scale_color_manual(values=c("#B92626", "#5AA9EF", "#D08B22")) +
  theme(text = element_text(size = 12), 
        plot.title = element_text(hjust = 0.5, size = 12),
        axis.text.x = element_text(size = 12),
        legend.text = element_text(size = 12),
...
graphSurprisalCorrExp1

ggsave(filename = "../graphs/surprisal_correlation_exp1.pdf", plot = graphSurprisalCorrExp1,
       width = 5, height = 4, device = "pdf")



graphSurprisalCorrExp2 <- dfCorr %>% 
  filter(experiment == 2) %>%
  ggplot(aes(x=surprisal, y=correlation, color=determiner, shape = size,group=1)) + 
  geom_point(size = 4) +
  geom_smooth(method="lm",se=F,color="gray80",alpha=.5) +
...
graphSurprisalCorrExp2

ggsave(filename = "../graphs/surprisal_correlation_exp2.pdf", plot = graphSurprisalCorrExp2,
       width = 5, height = 4, device = "pdf")

exp.labs <- c("Exp. 1 (no exposure)", "Exp. 2 (exposure)")
names(exp.labs) <- c("1", "2")

graphSurprisalCorrExpFacet <- dfCorr %>% 
  ggplot(aes(x=surprisal, y=correlation, color=determiner, shape = size,group=1)) + 
  facet_grid(. ~ experiment) + 
  facet_grid(. ~ experiment, 
             scale = "free",
...
graphSurprisalCorrExpFacet

ggsave(filename = "../graphs/surprisal_correlation_exp_facet.pdf", plot = graphSurprisalCorrExpFacet,
       width = 5, height = 4, device = "pdf")


graphSurprisalCorr2022Talk <- dfCorr %>% 
  filter(experiment == 2) %>%
  ggplot(aes(x=surprisal, y=correlation, color=determiner, shape = size,group=1)) + 
  facet_grid(. ~ experiment, 
             scale = "free",
...
graphSurprisalCorr2022Talk

ggsave(filename = "../graphs/surprisal_correlation_2022.pdf", plot = graphSurprisalCorr2022Talk,
       width = 5, height = 4, device = "pdf")
