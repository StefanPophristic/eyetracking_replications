...
# Run 3 only included pragmatic bad condition
# Pragmatic good condition data is from run 2

df_pragBad = read_csv("../data/3_incremental_ryskin-trials.csv")
df_pragGood = read_csv("../../run_2/main/data/1_incremental_ryskin_main-trials.csv") %>%
  filter(pragContext == "good")

df <- rbind(df_pragBad, df_pragGood)
...
length(unique(df$workerid)) #58 participants
...
# All trial types have a competitor except for semantic-control and semantic-contrast fillers
df = df %>% 
  separate(target_pic, into = c("noun","feature"), sep="[_.]", remove=F, extra="drop") %>% 
  mutate(loc_competitor_pic = case_when(cond == "semantic_contrast" ~ "NA",
                                        cond == "semantic_control" ~ "NA",
                                        TRUE ~ loc_contrast))

# trialType == "train" & pragContext == "bad" ~ "NA",
...
df %>% 
  ...
  count()
#   proliferate cond      pragContext     n
# 1 List1                 bad            15
# 2 List1                 good           14
# 3 List2                 bad            14
# 4 List2                 good           15
...
df_merged_safe <- df_merged


propSelections <- df_merged %>%
  filter(trialType == "train" & window == "adjective" & pragContext == "unreliable" & Region == "competitor") %>%
  unique()
...
propSelections2 <- df_merged %>%
  filter(trialType == "train" & window == "adjective" & pragContext == "unreliable" & Region == "competitor") %>%
  unique()

# Check the above parameter settings for d_test
# do we see any selections? 
propSelections3 <- d_test %>%
  select (click_adj) %>%
  filter(trialType == "train" & pragContext == "unreliable")
