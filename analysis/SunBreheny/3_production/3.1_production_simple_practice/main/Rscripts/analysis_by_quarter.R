...
dfQuarter <- dfExp %>%
  mutate(quarter = case_when(trial_number <= 12 ~ 1,
                             trial_number >= 13 & trial_number <=24 ~ 2,
                             trial_number >= 25 & trial_number <=36 ~ 3,
                             trial_number >= 37 ~ 4))
...
genderQ <- dfQuarter %>%
  group_by(target_figure3, genderUsed, size, condition, quarter) %>%
  count()
...
genderQ <- left_join(genderQ, genderTempQ, by = c("target_figure3", "size", "condition", "quarter")) %>%
...
genderQ <- mutate_at(genderQ, "targetUtteranceN", ~replace(., is.na(.), 0))
...
surprisalGenderQ <- genderQ %>%
  group_by(target_figure3, size, condition, quarter) %>%
...
# based off of size and target figure conditions
detQ <- dfQuarter %>%
  group_by(condition, size, target_figure3, detUsed, quarter) %>%
  count()
...
detQ <- left_join(detQ, detTempQ, by = c("condition", "size", "target_figure3", "quarter")) %>%
  select(-c(detUsed.y, n.y)) %>%
  rename(determiner = detUsed.x, n = n.x)

# If no target utterances were observed, they will come up as NA in the targetUtteranceN
# column --> replace those values with 0
detQ <- mutate_at(detQ, "targetUtteranceN", ~replace(., is.na(.), 0))

surprisalDetQ <-detQ %>%
  group_by(condition, size, target_figure3, quarter) %>%
...
# based off of size and target figure conditions
nounQ <- dfQuarter %>%
  group_by(condition, size, target_figure3, target_object3, correctNoun, quarter) %>%
  count()
...
nounQ <- left_join(nounQ, nounTempQ, by = c("target_object3", "target_figure3", "size", "condition", "quarter")) %>%
  select(-c(correctNoun.y, n.y)) %>%
  rename(correctNoun = correctNoun.x, n = n.x)

# If no target utterances were observed, they will come up as NA in the targetUtteranceN
# column --> replace those values with 0
nounQ <- mutate_at(nounQ, "targetUtteranceN", ~replace(., is.na(.), 0))
...
# Surprisal = -log(P(Utterance))
# P(Utterance) = [number of instances of that utterance given a condition] / [total number of utterances given a condition]
surprisalNounQ <- nounQ %>%
  group_by(target_object3, size, condition, target_figure3, quarter) %>%
  summarize(surprisal = -log2(targetUtteranceN / sum(n))) %>%
  distinct()


#######
# Plot surprisal by Quarter
#######

# Gender Surprisal
graphGenderSurprisalQuarter <- surprisalGenderQ %>%
  ggplot(aes(fill = condition, y=surprisal, x=target_figure3, color = condition)) + 
  facet_grid(size ~ quarter) +
  geom_point(size = 4) +
...
graphGenderSurprisalQuarter
ggsave("../graphs/gender_surprisal_by_quarter.pdf", plot = graphGenderSurprisalQuarter, width=8,height=8)

# Determiner Surprisal
surprisalDetQ$condition <- surprisalDetQ$condition %>%
  factor(levels = c('all', 'some', 'num'))
surprisalDetQ$size <- surprisalDetQ$size %>%
  factor(levels = c('big', 'small'))
surprisalDetQ$target_figure3 <- surprisalDetQ$target_figure3 %>%
  factor(levels = c('boy', 'girl'))

graphSurprisalDetQuarter <- surprisalDetQ %>%
  select(condition, size, target_figure3, surprisal) %>%
  ggplot(aes(fill=size, y=surprisal, x=condition, color=size)) + 
  facet_grid(target_figure3 ~ quarter) +
  geom_point(size = 4) +
...
graphSurprisalDetQuarter
ggsave("../graphs/det_surprisal_by_quarter.pdf", plot = graphSurprisalDetQuarter, width=8,height=8)

# Plot noun surprisal
graphSurprisalNounQuarter <- surprisalNounQ %>%
...
graphSurprisalNounQuarter
ggsave("../graphs/noun_surprisal_by_quarter.pdf", plot = graphSurprisalNounQuarter, width=8,height=8)
...
#############################################
#
# Prep the data for the Cogsci graph
#
#############################################
#############################################

# These graphs were not used in the cogsci paper
# but this section creates the necessary csv files which are used 
# to create the cogsci grpahs
...
dfDetQ <- dfDetQ %>%
  group_by(size, noun, detUsed, quarter) %>%
  count() %>%
  ungroup()

# Add in all the size x noun x determiner x quarter used combinations which were not observed
allCombinationsQ <- unique(expand.grid(dfDetQ$size, dfDetQ$noun, c("all", "some", "num", "noDet", "other"), c(1, 4)))
allCombinationsQ <- allCombinationsQ %>%
  rename(size = Var1, noun = Var2, detUsed = Var3, quarter = Var4)

dfDetQ <- right_join(dfDetQ, allCombinationsQ, by = c("size", "noun", "detUsed", "quarter"))
...
dfDetQ <- merge(dfDetQ, totalSumsQ, by = c("size", "noun"))
...
mQ = lmer(surprisal ~ detUsed*size + (1|noun), data=tmpQ)

summary(mQ)

# # Graph it

surprisalMeanAndCIQ <- dfDetQ %>%
  group_by(size,detUsed, quarter) %>%
  summarize(
...
write.csv(surprisalMeanAndCIQ, "../data/simple_surprisalMeanAndCI_by_quarter.csv", row.names = FALSE)


# Another script will make teh final graph
# this is just for visualization purposes for us
graphSurprisalDetByNounByQuarter <- surprisalMeanAndCIQ %>%
  ungroup() %>%
  ...
  xlab("Determiner")
graphSurprisalDetByNounByQuarter

ggsave(filename = "../graphs/det_surprisal_by_noun_quarter.pdf", plot = graphSurprisalDetByNounByQuarter,
...
# Add quarters to g dataframe
# the column "unique" codes for the trial number
g <- g %>%
...
# The selection data doesn't have trial numbers, so I'm going to assume
# that the order that data is written in the CSV file is the order of trials
# add trial numbers
# These trials differ from dataset g because g doesn't include 12 filler items
# for consistency, we get rid of the 12 filler items and calculate the quarter based
# just on the experimental items
# The initial six practice tirals will be given trial number 0
s <- s %>%
  filter(ExpFiller=="Exp") 
s <- s %>%
  mutate(trial_number = rep(c(1:36), nrow(s)/36)) %>%
  mutate(quarter = case_when(
    trial_number %in% c(1:8) ~ 1,
    trial_number %in% c(9:16) ~ 2,
    trial_number %in% c(17:24) ~ 3,
    trial_number %in% c(25:36) ~ 4))


s_unique <- s %>% select(determiner, target_figure3, quarter, size) %>% unique()
g_unique <- g %>% select(determiner, target.figure, quarter, size) %>% unique()

view(s_unique)
# Example from s_unique:
# det  gender quarter size
# some boy    1       big
# some boy    4       small
# some boy    1       small
# some boy    4        big
view(g_unique)
# Example from g_unique:
# det  gender quarter size
# some boy    1       small
# some boy    4        big
...
selection = s %>%
  ...
  select(workerid,Prime,condition,determiner,size,target_figure3,window,location,target1,target2,competitor1,competitor2,item, quarter) %>%
  ...
  group_by(determiner,size,window,item, target_figure3, quarter) %>%
...
gaze =  g %>%
  ...
  select(Prime,condition,determiner,size,targetlook,target.figure, competitorlook,residuelook,whichword,item, quarter) %>%
  ...
  group_by(determiner,size,window,item, target.figure, quarter) %>%
...
df = merge(selection, gaze, by=c("determiner","size","window","item", "gender", "quarter"))
...
toplot = longer_looks %>% 
  left_join(longer_selections,by=c("determiner","size","window","Region","item", "gender", "quarter")) %>% 
...
# window          quarter Correlation       P
# <fct>             <dbl>       <dbl>   <dbl>
#   1 baseline              1        0.49 0.00001
# 2 baseline              4        0.54 0      
# 3 gender                1        0.59 0      
# 4 gender                4        0.76 0      
# 5 determiner+name       1        0.84 0      
# 6 determiner+name       4        0.84 0      
# 7 noun                  1        0.94 0      
# 8 noun                  4        0.94 0
...
cors_determiner = toplot %>% 
  ...
  group_by(determiner, gender, quarter, size) %>% 
  summarize(Correlation=round(cor.test(prop_selections,prop_looks)$estimate,2),P=round(cor.test(prop_selections,prop_looks)$p.value,5))
cors_determiner
# determiner gender quarter size  Correlation       P
# 1 all        boy          1 big          0.85 0.00049
# 2 all        boy          4 small        0.79 0.00009
# 3 all        girl         1 big          0.89 0.00009
# 4 all        girl         4 small        0.72 0.00079
# 5 some       boy          1 small        0.63 0.0281 
# 6 some       boy          4 big          0.83 0.00002
# 7 some       girl         1 small        0.68 0.0144 
# 8 some       girl         4 big          0.79 0.00215
# 9 number     boy          1 big          0.99 0.0857 
# 10 number     boy          1 small        0.96 0.00004
# 11 number     boy          4 big          0.92 0.00831
# 12 number     boy          4 small        0.92 0.00003
# 13 number     girl         1 big          0.91 0.00057
# 14 number     girl         1 small        1    0.0327 
# 15 number     girl         4 big          0.94 0.00015
# 16 number     girl         4 small        0.94 0.00592

# correlation between eye movement and decision task data separately by condition within noun window
cors_noun = toplot %>% 
  filter(window == "noun") %>% 
  group_by(determiner, size, gender, item, quarter) %>% 
  summarize(Correlation=round(cor.test(prop_selections,prop_looks)$estimate,2),P=round(cor.test(prop_selections,prop_looks)$p.value,5))
cors_noun
...
cors_gender = toplot %>% 
  ...
  group_by(determiner, size, gender, quarter) %>% 
  summarize(Correlation=round(cor.test(prop_selections,prop_looks)$estimate,2),P=round(cor.test(prop_selections,prop_looks)$p.value,5))
cors_gender
...
surprisalDet <- surprisalDetQ %>%
  rename(gender = target_figure3) %>%
  mutate(condition=fct_recode(condition, "number"="num"))

# Merge surprisal and correlation data
finalCorrelationDetDF <- surprisalDet %>%
  select(condition, size, gender, quarter, surprisal) %>%
  merge(cors_determiner, by = c("condition", "size", "gender", "quarter"))

write.csv(finalCorrelationDetDF, "../data/simple_finalCorrelationDetDF_quarter.csv", row.names = FALSE)
